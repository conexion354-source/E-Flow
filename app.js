...
function getApiStatusElement() {
  return document.getElementById("apiStatusText");
}
...
function showStatus(message) {
  ...
  const apiText = getApiStatusElement();

  if (!message) {
    if (banner) banner.classList.add("hidden");
    if (apiText) apiText.textContent = "Conectado correctamente.";
    return;
  }
  ...
  if (apiText) apiText.textContent = message;
}
...
async function cargar() {
  try {
    ...
    if (!currentMonth) {
      ...
      currentMonth = first
        ? new Date(
            parseDateSafe(first.fechaPago).getFullYear(),
            parseDateSafe(first.fechaPago).getMonth(),
            1
          )
        : new Date();
    }
...
function renderMiniList(id, items, filter) {
  ...
  items.forEach((c) => {
    ...
    d.addEventListener("click", () => {
      filteredDatos = [c];
      document.getElementById("chequesTitle").textContent =
        filter === "rechazados" ? "Detalle rechazado" : "Detalle anulado";
...
function renderCalendarioMensual() {
  ...
  aPagar.forEach((c) => {
    ...
    if (!fecha) return;
    if (
      fecha.getMonth() !== currentMonth.getMonth() ||
      fecha.getFullYear() !== currentMonth.getFullYear()
    ) return;

    const day = fecha.getDate();
    if (!map[day]) {
      map[day] = { cant: 0, monto: 0, items: [], minDiff: 999 };
    }
  ...
  for (let day = 1; day <= lastDay.getDate(); day++) {
    ...
    if (info) {
      ...
      cell.addEventListener("click", () => {
        ...
        document.getElementById("chequesTitle").textContent =
          `Cheques del ${day}/${currentMonth.getMonth() + 1}/${currentMonth.getFullYear()}`;
        document.getElementById("chequesSubtitle").textContent = "Vista detallada del día";
        renderCheques();
        detail.innerHTML =
          `<strong>${day}/${currentMonth.getMonth() + 1}/${currentMonth.getFullYear()}</strong><p class="meta">${info.cant} cheques · ${formatMoney(info.monto)}</p>`;
        switchToView("chequesView");
      });
    } else {
...
function renderCheques() {
  ...
  if (!filteredDatos.length) {
    lista.innerHTML =
      '<article class="cheque"><div class="prov">No hay cheques para mostrar</div><div class="meta">No hay registros para este filtro.</div></article>';
    return;
  }
...
async function initPush() {
  try {
    ...
    if (existingToken) {
      ...
      if (tokenBox) tokenBox.value = existingToken;
      setApiStatus("Push listo. Ya existe un token guardado en este dispositivo.");
    }
  } catch (e) {
...
async function requestNotifications() {
  ...
  try {
    ...
    // Usar SOLO el service worker principal
    ...
    if (!token) {
      throw new Error("Firebase no devolvió token.");
    }
...
window.addEventListener("beforeinstallprompt", (e) => {
  ...
  ["installBtnTop", "installBtnInside", "installBtnLogin"].forEach((id) => {
    const el = document.getElementById(id);
...
window.addEventListener("load", async () => {
  ...
  if (storedToken) {
    document.getElementById("pushToken").value = storedToken;
  }

  if (localStorage.getItem("eflow_installed") === "yes") hideInstallButtons();

  if (
    localStorage.getItem("eflow_notifications") === "on" ||

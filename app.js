...
function dueCountdown(check) {
  ...
  const days = daysBetween(today, check.paymentDate);
  ...
  if (days < 0) {
    const expiryDate = addDaysToDate(check.paymentDate, 30);
    const expiryDays = daysBetween(today, expiryDate);
    if (expiryDays < 0) {
      return { days: expiryDays, label: `EXP ${labelDays(Math.abs(expiryDays))}`, className: "due-red" };
    }
    if (expiryDays === 0) return { days: expiryDays, label: "EXP HOY", className: "due-red" };
    return { days: expiryDays, label: `EXP ${labelDays(expiryDays)}`, className: urgencyClass(expiryDays) };
  }

  if (days === 0) return { days, label: "HOY", className: "due-red" };
  return { days, label: labelDays(days), className: urgencyClass(days) };
}

...
function openChequeInNewTab(button){
  try{
    const card = button.closest(".cheque");
...
<style>
body{margin:0;padding:24px;background:#f3f5f8;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;color:#111827}
.wrap{max-width:760px;margin:0 auto}
.cheque{background:#fff;border-radius:18px;padding:14px;border:1px solid #e5e7eb;box-shadow:0 10px 30px rgba(17,24,39,.08)}
.prov{font-weight:800;font-size:18px}
.amount{font-weight:800;color:#059669;font-size:18px}
.row{display:flex;justify-content:space-between;gap:10px}
.meta{margin-top:8px;color:#4b5563;font-size:14px;line-height:1.6}
</style>
</head>
<body><div class="wrap">${clone.outerHTML}</div></body></html>`;
...
function hideInstallButtons(){
  const a=document.getElementById("installCard"); if(a) a.classList.add("hidden");
  const b=document.getElementById("installConfigRow"); if(b) b.classList.add("hidden");
  const c=document.getElementById("installBtnLogin"); if(c) c.classList.add("hidden");
}
...
window.openChequeInNewTab = openChequeInNewTab;
...
window.addEventListener("beforeinstallprompt", e=>{
  ...
  ["installBtnTop","installBtnInside","installBtnLogin"].forEach(id => { const el=document.getElementById(id); if(el) el.classList.remove("hidden"); });
});
...
const ib1=document.getElementById("installBtnTop"); if(ib1) ib1.addEventListener("click", installApp);
const ib2=document.getElementById("installBtnInside"); if(ib2) ib2.addEventListener("click", installApp);
const ib3=document.getElementById("installBtnLogin"); if(ib3) ib3.addEventListener("click", installApp);

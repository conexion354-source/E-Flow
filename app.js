...
function dueCountdown(check) {
  const days = daysBetween(getTodayString(), check.paymentDate);
  if (days < 0) {
    const count = Math.abs(days);
    return { days, label: `Vencido hace ${count} ${count === 1 ? "dia" : "dias"}`, className: "due-red" };
  }
  if (days === 0) return { days, label: "Vence hoy", className: "due-red" };
  if (days <= 5) return { days, label: `Faltan ${days} ${days === 1 ? "dia" : "dias"}`, className: "due-red" };
  if (days <= 10) return { days, label: `Faltan ${days} dias`, className: "due-orange" };
  if (days <= 15) return { days, label: `Faltan ${days} dias`, className: "due-yellow" };
  return { days, label: `Faltan ${days} dias`, className: "due-green" };
}

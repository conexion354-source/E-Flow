...
function dueCountdown(check) {
  const today = getTodayString();
  const daysUntilPayment = daysBetween(today, check.paymentDate);
  const labelDays = (value) => `${value} ${value === 1 ? "DIA" : "DIAS"}`;
  const urgencyClass = (value) => {
    if (value <= 5) return "due-red";
    if (value <= 10) return "due-orange";
    if (value <= 15) return "due-yellow";
    return "due-green";
  };

  if (daysUntilPayment < 0) {
    const expirationDate = addDaysToDate(check.paymentDate, 30);
    const daysUntilExpiration = daysBetween(today, expirationDate);
    if (daysUntilExpiration < 0) {
      return { days: daysUntilExpiration, label: `EXPIRADO ${labelDays(Math.abs(daysUntilExpiration))}`, className: "due-red" };
    }
    if (daysUntilExpiration === 0) return { days: daysUntilExpiration, label: "EXP HOY", className: "due-red" };
    return { days: daysUntilExpiration, label: `EXP ${labelDays(daysUntilExpiration)}`, className: urgencyClass(daysUntilExpiration) };
  }

  if (daysUntilPayment === 0) return { days: daysUntilPayment, label: "HOY", className: "due-red" };
  return { days: daysUntilPayment, label: labelDays(daysUntilPayment), className: urgencyClass(daysUntilPayment) };
}

function renderDateSummary(check) {
  const due = dueCountdown(check);
  return `
    <div class="date-summary">
      <span class="due-pill ${due.className}">${escapeHtml(due.label)}</span>
      <div>
        <strong>Emision: ${formatDate(check.issueDate)}</strong>
        <span>Cobro/Pago: ${formatDate(check.paymentDate)}</span>
      </div>
    </div>
  `;
}
...
function resetListFilters() {
  state.listFilters = { direction: "todos", term: "", date: null, status: "pendiente", checkId: null };
}
...
function dashboardDueLabel(check, today) {
  if (check.paymentDate < today && isPending(check)) {
    return "Aún no depositado por destinatario";
  }
  if (check.paymentDate === today) return "Hoy";
  return "Mañana";
}
...
function renderDashboardView() {
  ...
  return `
    <section>
      ...
      <div class="widgets">
        <article class="widget is-red" data-dashboard-direction="emitido">
          <div class="label">Pagos de hoy</div>
          <h2>${formatCurrency(stats.totalEmitidosHoy)}</h2>
          <small>${stats.emitidosHoy.length} a cubrir / vencidos</small>
        </article>
        ...
        <div class="panel-body">
          ${
            stats.criticos.length
              ? stats.criticos
                  .map((check) => {
                    const dueLabel = dashboardDueLabel(check, today);
                    return `
          ...
          <div class="mobile-cards">
            ${rows
              ...
              <article class="mobile-card">
                ...
                <div class="mobile-card-meta">${escapeHtml(check.bank)} · Nº ${escapeHtml(check.checkNumber)}</div>
                ${renderDateSummary(check)}

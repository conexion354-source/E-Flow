...
async function sendApiMutation(operation, payload) {
  ...
  if (text) {
    try {
      ...
      if (isHtmlResponse(text)) throw new Error("La API devolvió HTML en lugar de JSON.");
    }
  }
...
function resetListFilters() {
  state.listFilters = { direction: "todos", term: "", date: null };
}
...
function bindRegisterEvents() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      savedRemotely = false;
      showStatus(`No se pudo guardar en la API compartida (${error?.message || "error"}). Se guardó localmente.`);
    }
...
async function updateCheckStatus(checkId, newStatus) {
  ...
  if (check.source === "remote") {
    try {
      ...
      saveStatusOverrides();
      showStatus(`No se pudo sincronizar el estado en la API (${error?.message || "error"}). Guardado local.`);
    }
  } else {
...
async function deleteCheck(checkId) {
  ...
  if (check.source === "remote") {
    try {
      ...
      showStatus(`No se pudo eliminar en la API (${error?.message || "error"}). Se ocultó solo en este dispositivo.`);
    }
  } else {
...
function bindListEvents() {
  ...
  document.querySelectorAll("[data-status-id]").forEach((select) => {
    select.addEventListener("change", async () => {
      ...
      try {
        await updateCheckStatus(select.dataset.statusId, select.value);
      } finally {

...
const NAV_ITEMS = [
  ...
  { id: "calendar", label: "Calendario", code: "CA" },
  { id: "entities", label: "Directorio", code: "DI" },
  { id: "list", label: "Historial", code: "HI" },
  { id: "reports", label: "Reportes", code: "RE" }
];
...
function updateForceMobile(enabled) {
  state.forceMobile = Boolean(enabled);
...
function renderDesktopNav() {
  ...
  nav.querySelectorAll("[data-nav-view]").forEach((btn) => {
    btn.addEventListener("click", () => setView(btn.dataset.navView));
  });
...
function renderMobileNav() {
  ...
  nav.querySelectorAll("[data-mobile-view]").forEach((btn) => {
    btn.addEventListener("click", () => setView(btn.dataset.mobileView));
  });
}
    ...
    <section>
      ...
      <article class="panel">
        <header class="panel-head">
          ...
          <button class="btn-soft" type="button" id="dashboardGoList">Ver historial</button>
        </header>
...
function getFilteredChecks() {
  ...
  return [...state.checks]
    .filter((check) => {
      if (state.filterDirection !== "todos" && check.direction !== state.filterDirection) return false;
...
function statusSelect(check) {
  return `
    <select data-status-id="${escapeHtml(check.id)}" class="badge ${statusClass(check.status)}">
      ${STATUS_OPTIONS.map((opt) => `<option value="${opt.value}" ${opt.value === check.status ? "selected" : ""}>${opt.label}</option>`).join("")}
    </select>
  `;
}
    ...
    <section>
      ...
      <div class="controls">
        <select id="directionFilter">
          <option value="todos" ${state.filterDirection === "todos" ? "selected" : ""}>Todos</option>
          <option value="emitido" ${state.filterDirection === "emitido" ? "selected" : ""}>Solo emitidos</option>
          <option value="recibido" ${state.filterDirection === "recibido" ? "selected" : ""}>Solo recibidos</option>
        </select>

        <div class="search">
          <input id="searchInput" type="text" value="${escapeHtml(state.searchTerm)}" placeholder="Buscar por banco, contacto o número" />
        </div>
      </div>
              ...
              <tbody>
                ${rows
                  ...
                  <tr>
                    ...
                    <td>${statusSelect(check)}</td>
                    <td>
                      <div class="actions">
                        ${check.photoUrl ? '<button type="button" class="action-btn photo">IMG</button>' : ""}
...
function bindDashboardEvents() {
  document.querySelectorAll("[data-dashboard-open]").forEach((card) => {
    card.addEventListener("click", () => {
      state.filterDirection = card.dataset.dashboardOpen;
      state.searchTerm = "";
      setView("list");
    });
  });
...
function bindListEvents() {
  ...
  if (filterSelect) {
    filterSelect.addEventListener("change", () => {
      state.filterDirection = filterSelect.value;
      renderApp();
    });
  }
  ...
  document.querySelectorAll("[data-status-id]").forEach((select) => {
    select.addEventListener("change", () => {
      updateCheckStatus(select.dataset.statusId, select.value);
    });
  });

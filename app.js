...
async function cargar() {
  try {
    ...
    if (!Array.isArray(data)) {
      throw new Error("La API no devolvió una lista de cheques.");
    }

    masterDatos = data;
    filteredDatos = [...masterDatos];
    if (!masterDatos.length) {
      showStatus("No se encontraron registros. Revisá la fila inicial o el acceso de la hoja en Apps Script.");
    } else {
      showStatus("");
    }
    renderAll();
    ...
    filteredDatos = [];
    renderAll();
    showStatus("No se pudieron cargar los datos. Revisá Google Apps Script: el documento puede no existir o no tener acceso.");
  }
}

window.FIREBASE_CONFIG = {
  apiKey: "AIzaSyD1s9NUDbZlfOx3DD6Q7t19qZjrDQ5NFek",
  authDomain: "e-flow-96cdc.firebaseapp.com",
  projectId: "e-flow-96cdc",
  storageBucket: "e-flow-96cdc.firebasestorage.app",
  messagingSenderId: "876330685560",
  appId: "1:876330685560:web:66770d640c34c8565ac98d",
  measurementId: "G-CWQ0GJK5KD"
};

window.FIREBASE_OPTIONS = {
  // Colección principal para sincronización en tiempo real de cheques
  checksCollection: "cheques",
  // Documento compartido para bancos y contactos. Usa una colección ya habilitada por reglas.
  entitiesCollection: "cheques_photos",
  entitiesDoc: "entities",
  // Carpeta destino para fotos de cheques en Firebase Storage
  storageFolder: "cheques"
};
